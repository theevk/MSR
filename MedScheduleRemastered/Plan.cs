...
namespace MedScheduleRemastered
{
    public class Plan
    {
        ...
        public void EvaluatePlan()
        {
            ...
            int minNurseShiftDifference = 1000;
            int maxNurseShiftDifference = 0;
            double tempFitness;
        ...
        public void Printplan()
        {
            foreach(Day day in _days)
            {
                foreach(Shift shift in day.FullDay)
                {
                    ...
                    {
                        Console.WriteLine(nurse.ToString());
                    }
                }
            }
            Console.WriteLine(_fitnessScore.ToString());
            Console.WriteLine("Nightviolations: {0}, Freeshiftviolations: {1}, Averageshiftdifference: {2}", _nightViolations, _freeShiftViolations, _averageShiftDifference);
        }
